...
| 에러 코드 | 메시지 | 발생 엔드포인트 | 트리거 조건 |
|-----------|--------|----------------|------------|
| `RENTAL_EXPIRED` | 대여 기간이 만료되었습니다. 구매 페이지에서 다시 구매해주세요 | `GET /series/{id}/purchase-check` | rental 구매 후 48시간 경과 |

> **Frontend 동작**: `RENTAL_EXPIRED` 수신 시 자동으로 구매 페이지(`/purchase/:id`)로 이동
...
## 정책 결정 완료 항목 (2026-03-21 확정)

| # | 시나리오 | 결정 | 구현 상태 |
|---|---------|------|----------|
| 1 | 대여 만료된 콘텐츠 재생 시도 | 403 `RENTAL_EXPIRED` → Frontend가 구매 페이지로 자동 이동 | ✅ 구현 |
| 2 | 이미 구매한 시리즈 재구매 시도 | 무시 (포인트 차감 없이 기존 구매 정보 반환, `points_used=0`) | ✅ 구현 |
| 3 | 동일 시리즈 찜 중복 추가 | `ON CONFLICT DO NOTHING` (성공 반환) — 현행 유지 | ✅ 기존 |
| 4 | WebSocket 연결 끊김 시 광고 상태 | 재연결 시 미제거 광고 복원 전송 | ✅ 구현 |
