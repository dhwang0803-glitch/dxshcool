...
## 2차 테스트 결과 (work_mem=256MB 적용)

| ID | 1차 | 2차 | 변화 | 판정 |
|----|-----|-----|------|------|
| P01 | 1,272ms | 979ms | -23% (disk sort 제거) | FAIL |
| P02 | 15,920ms | 9,559ms (일반 VOD) | 참고값 | FAIL |
| P04 | 23,292ms | 15,703ms | -33% (external merge→in-memory) | FAIL |

**주요 변화**: work_mem 증가로 P04의 external merge sort(Disk) → in-memory HashAggregate로 전환. 디스크 I/O 제거로 실행 시간 단축됨. 단, 근본적인 IO 병목(1코어 + cold cache)은 해소되지 않음.
