...
class ShelfBuilder(HybridBase):
    ...
    def _build_tag_vod_cache(conn, tag_list: list[tuple]) -> dict[tuple, list]:
        ...
        with conn.cursor() as cur:
            cur.execute(
                """
                SELECT t.tag_category, t.tag_value, t.vod_id_fk, t.confidence,
                       v.ct_cl, v.series_nm
                FROM (
            ...
            for cat, val, vod_id, confidence, ct_cl, series_nm in cur.fetchall():
        ...
        for user_id in user_ids:
            ...
            for cat, slots in _CATEGORY_SLOTS.items():
                ...
                for _cat_rank, val, aff in candidates:
                    ...
                    tag_vods: list[tuple] = []  # (vod_id, confidence)
                    seen_series: set = set()
                    ...
                    for vod_idx, (vod_id, confidence) in enumerate(tag_vods, 1):
                        all_rows.append((
                            ...
                            vod_id, vod_idx, round(min(confidence, 1.0), 6),
                        ))
