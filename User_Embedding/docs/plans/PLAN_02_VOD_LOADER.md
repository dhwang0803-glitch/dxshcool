...
**목표**: DB `vod_embedding`(CLIP 512) + `vod_meta_embedding`(METADATA 384) 두 테이블에서 임베딩을 읽어 concat → 896차원 VOD 결합 벡터 딕셔너리 반환

> **범위 주의**: 개별 벡터의 L2 정규화는 `VOD_Embedding` 브랜치 담당.
> 이 브랜치는 DB에 이미 정규화된 벡터가 저장되어 있다고 가정하고 읽기만 한다.
...
| 항목 | 내용 |
|------|------|
| **입력 (CLIP)** | DB `vod_embedding` 테이블 — CLIP ViT-B/32, 512차원 (정규화 완료) |
| **입력 (METADATA)** | DB `vod_meta_embedding` 테이블 — paraphrase-multilingual-MiniLM-L12-v2, 384차원 (정규화 완료) |
...
# asset_id 기준 inner join 후 단순 concat
combined = np.concatenate([
    clip_vec,   # 512차원 (DB에서 읽은 값, 이미 정규화됨)
    meta_vec,   # 384차원 (DB에서 읽은 값, 이미 정규화됨)
])  # → 896차원
```
