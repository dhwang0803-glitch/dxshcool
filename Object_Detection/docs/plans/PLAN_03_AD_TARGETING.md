...
### CLIP (현재 영어 쿼리 — threshold 0.25)

| 탐지 가능 | 탐지 불가 (쿼리 없음) |
|---------|-----------------|
| beach seaside ocean ✅ | pineapple ❌ |
| Korean food, BBQ, seafood ✅ | 굴비/조기 (dried corvina) ❌ |
| mountain hiking ✅ | 대게 (snow crab) ❌ |
| home appliance, furniture ✅ | 유럽 배경 ❌ |
| | 동남아 휴양지 ❌ |
...
한국어 fine-tuning 모델 교체로 한국 특산품 인식 정확도 향상.

```bash
pip install git+https://github.com/jaketae/koclip.git
```

쿼리 복원 (한국어):
- "굴비", "조기", "대게", "한우", "파인애플"

**기대 효과**: 영어 쿼리 대비 한국 특산품 concept 간 점수 분산 증가
...
- [ ] CLIP 쿼리 보강 (`config/clip_queries.yaml`) — 지방특산물·여행지·홈쇼핑_확장 추가
- [ ] `ad_category` 컬럼 산출물 추가
- [ ] 파일럿 10건 재실험 → 지방특산물·여행지 concept 탐지 확인
- [ ] KoCLIP 전환 실험 (영어 쿼리 A/B 비교)
