...
## KoCLIP A/B 실험 (2026-03-15 오후)

### 배경
`jaketae/koclip` pip 설치 불가(setup.py 미존재). `clip-ViT-B-32-multilingual-v1`으로 대체.
multilingual 모델은 텍스트 전용 인코더 → 이미지 인코딩 불가 → 듀얼 모델 구조 채택.

**구현**: `clip_scorer.py` — `_is_multilingual` 분기 추가
- 이미지: `clip-ViT-B-32`
- 텍스트: `clip-ViT-B-32-multilingual-v1`

### 실험 1 — 랜덤 VOD (서로 다른 10건씩)

| 지표 | EN | KO |
|------|----|----|
| 총 건수 | 408 | 7,878 |
| 고유 concept | 19/43 | 34/43 |
| std | 0.010 | 0.013 |

→ 서로 다른 VOD라 공정 비교 불가. 재실험.

### 실험 2 — 동일 VOD 10건 (공정 비교)
...
- **커버리지**: KO 33개 > EN 20개 → KO 우위
- **건수 차이**: KO 2.8배 더 많음 → threshold 상향으로 제어 가능
- **한국 특산물**: KO에서 "굴비 먹는 식사 장면", "대게 해산물", "장어구이" 직접 탐지

### threshold 튜닝 결과

| threshold | 건수 | concept | 판정 |
|-----------|------|---------|------|
| KO 0.25 | 3,871 | 33개 | 너무 많음 |
| EN 0.25 | 1,385 | 20개 | 기준 |
| **KO 0.26** | **1,925** | **23개** | **최적 채택** |
| KO 0.27 | 889 | 14개 | 너무 적음 |

### 채택 결정 ✅

**`clip-ViT-B-32-multilingual-v1` + 한국어 쿼리 + threshold 0.26**

- `config/clip_queries.yaml` 한국어 쿼리로 교체 완료
- 이미지: `clip-ViT-B-32` / 텍스트: `clip-ViT-B-32-multilingual-v1` 듀얼 모델
- EN 대비 concept 커버리지 우위(23 vs 20), 건수 유사(1,925 vs 1,385)
...
| 작업 | 상태 |
|------|------|
| `phase3_ad_targeting_report.md` 작성 | 🔲 예정 |
