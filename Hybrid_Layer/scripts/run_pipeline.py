...
Usage:
    ...
    python Hybrid_Layer/scripts/run_pipeline.py --start-phase 3   # Phase 3~4만 실행
...
def main():
    ...
    parser.add_argument("--start-phase", type=int, default=1, choices=[1, 2, 3, 4],
                        help="시작 Phase (기본 1). 이전 Phase 결과가 DB에 있을 때 사용.")
    parser.add_argument(
    ...
    if args.start_phase <= 1:
        ...
        log.info("Phase 1 건너뜀 (--start-phase %d)", args.start_phase)
    ...
    if args.start_phase <= 2:
        ...
        log.info("Phase 2 건너뜀 (--start-phase %d)", args.start_phase)
    ...
    if args.start_phase <= 3:
        ...
        log.info("Phase 3 건너뜀 (--start-phase %d)", args.start_phase)
    ...
    for phase, key in [(1, "phase1"), (2, "phase2"), (3, "phase3"), (4, "phase4")]:
        if args.start_phase <= phase:
            log.info("  Phase %d: %10d rows", phase, results.get(key, 0))
    log.info("=" * 60)
