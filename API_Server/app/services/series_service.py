...
class SeriesService(BaseService):
    async def get_episodes(self, series_nm: str) -> list[dict]:
        """에피소드 목록 — DISTINCT ON(asset_nm) + provider 우선순위."""
    ...
    async def get_progress(self, user_id: str, series_nm: str) -> dict:
        """에피소드별 시청 진행 현황."""
        rows = await self.query(
            """
            SELECT ep.completion_rate, ep.watched_at, v.asset_nm
    ...
    async def update_progress(
        self, user_id: str, series_nm: str, asset_nm: str, completion_rate: int
    ) -> dict | None:
        """에피소드 시청 진행률 UPSERT."""
    ...
    async def check_purchase(self, user_id: str, series_nm: str) -> dict:
        """시리즈 구매 여부 + 만료 확인."""
    ...
    async def resolve_vod_id(self, series_nm: str, asset_nm: str) -> str | None:
        """series_nm + asset_nm → full_asset_id 조회."""
        row = await self.query_one(
            """
            SELECT full_asset_id FROM public.vod
            WHERE series_nm = $1 AND asset_nm = $2
    ...
    async def get_purchase_options(self, series_nm: str) -> dict:
        """구매 옵션 — FOD 시리즈는 무료."""
        row = await self.query_one(
            "SELECT asset_prod FROM public.vod WHERE series_nm = $1 LIMIT 1",
            series_nm,
        )
        if not row:
...
# 하위 호환
get_episodes = series_service.get_episodes
get_series_progress = series_service.get_progress
