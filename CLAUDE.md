...
| 광고 시스템 | VOD 음식/관광지 2종 인식(CLIP/STT/YOLO) → 음식: 제철장터 채널 연계(채널 이동/시청예약), 관광지: 지자체 광고 팝업(지역 축제·관광) |
...
| 브랜치 | 역할 | 주요 경로 |
|--------|------|-----------|
| `Object_Detection` | VOD 음식/관광지 2종 인식 + 지역명 추출 → parquet 산출물 (로컬 전용) | `Object_Detection/src/`, `Object_Detection/scripts/` |
| `Shopping_Ad` | Object_Detection 산출물 → 제철장터 상품 매칭 + 지자체 광고 연계 → `serving.shopping_ad` (VPC 적재) | `Shopping_Ad/src/`, `Shopping_Ad/scripts/` |
...
## 🚀 배포 규칙 (MANDATORY — 모든 브랜치 적용)

**배포 요청 시 반드시 `docs/DEPLOY_GUIDE.md`를 참조한다.**
...
### 배포 대상 판단 기준 (MANDATORY)

```
"배포해줘" / "dev 서버 업데이트" / "테스트 환경 반영"
  → vod-api-dev (개발) 배포

"운영 배포" / "릴리스" / "프로덕션 반영" + 조장 명시적 승인
  → vod-api (운영) 배포
```

**기본값은 항상 dev(`vod-api-dev`)이다. 운영 배포는 조장이 명시적으로 "운영 배포"라고 지시한 경우에만 수행한다.**

### 절대 금지 사항

- `release` 브랜치에 직접 커밋·머지 (조장 승인 없이)
- `vod-api` (운영 서비스)에 임의 배포
- dev 검증 없이 운영 배포
