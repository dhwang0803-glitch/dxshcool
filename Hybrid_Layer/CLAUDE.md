...
━━━ 입력 (기존 엔진 산출물) ━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  serving.vod_recommendation
    ...
    │     3배수 후보 추출 → ct_cl 기준 시리즈 중복제거 → 저장
    └── Vector_Search: user_id → VISUAL_SIMILARITY 타입 (source_vod_id=NULL)
          user_embedding CLIP([:512]) × VOD CLIP 코사인 유사도

  ※ CONTENT_BASED는 item-to-item (user_id 없음)
     → Phase 3 reranker의 유저별 후보 조회에 포함되지 않음
  ※ VISUAL_SIMILARITY는 user-to-item (user_id 있음)
     → Phase 3 reranker에서 CF + VISUAL_SIMILARITY 후보를 함께 리랭킹 가능
     → 현재는 CF(COLLABORATIVE) 후보만 Phase 3에 입력
