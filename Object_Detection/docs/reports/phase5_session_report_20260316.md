...
1. **TL.zip + VL.zip → Google Drive 업로드** (즉시 가능)
2. **Colab cell-8 실행** → Step 1~3 (클래스 스캔 + YOLO 변환 + data.yaml)
3. **로컬 aihubshell로 TS.z01 다운로드** → Drive 업로드 → Step 4 학습 실행
4. **학습 완료 후**: best.pt → `models/korean_food_v1_best.pt` → `config/detection_config.yaml` 경로 교체
5. **A/B 비교**: 기존 yolo11s.pt vs 파인튜닝 best.pt (한식 라벨 탐지 건수 비교)
