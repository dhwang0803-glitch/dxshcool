...
def load_manifest(manifest_path: str) -> list[dict]:
    """manifest CSV → 행 목록 반환. series_id 중복은 첫 번째만 사용."""
    seen = set()
    rows = []
...
def write_oci_map(out_path: str, records: list[dict]):
    """oci_map.csv 저장 (series_id, series_nm, oci_url)."""
    os.makedirs(os.path.dirname(out_path) or ".", exist_ok=True)
    with open(out_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=["series_id", "series_nm", "oci_url"])
        writer.writeheader()
...
def main():
    ...
    logger.info("매니페스트 로드: %d개 시리즈", len(rows))
        ...
        if args.skip_existing:
            try:
                if oci_uploader.object_exists(object_name):
                    ...
                    oci_records.append({"series_id": sid, "series_nm": series_nm, "oci_url": url})
                    skipped += 1
                    continue
    ...
    write_oci_map(args.out, oci_records)
    logger.info(
    ...
    if args.update_db and oci_records:
        ...
        mapping = {r["series_nm"]: r["oci_url"] for r in oci_records}
        conn = get_db_conn()
        try:
