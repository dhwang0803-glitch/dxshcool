...
VOD 재생 중 광고 팝업(지자체 광고 / 제철장터 연계)을 띄우기까지 3단계로 나뉜다.
...
### Phase B: 배치 매칭 + 광고 소재 적재 (Shopping_Ad)

> **2026-03-19 변경**: 홈쇼핑 크롤링 제거. 탐지 결과 기반 지자체 광고 + 제철장터 연계로 전환.

탐지 결과 매칭 + 광고 소재 적재로 `serving.shopping_ad`를 갱신한다.
...
│  ┌──────────────────────────┐     ┌──────────────────────────┐              │
│  │ 축제 리스트               │     │ 제철장터 상품 정보         │              │
│  │ (3~4월 지역 축제 등)      │     │ (채널 편성 등)            │              │
│  └────────────┬─────────────┘     └────────────┬─────────────┘              │
│               │ 수집                            │ 수집                       │
│               ▼                                ▼                            │
│  ┌──────────────────────────┐     ┌──────────────────────────┐              │
│  │ 생성형 AI 광고 제작       │     │ 제철장터 채널 매핑         │              │
│  │ → OCI 업로드             │     │                          │              │
│  └────────────┬─────────────┘     └────────────┬─────────────┘              │
│               │                                │                            │
│               └──────────────┬─────────────────┘                            │
...
│           │  2) 관광지/지역 인식 → 지자체 광고 매칭  │                          │
│           │  3) 음식 인식 → 제철장터 채널 연계       │                          │
│           │  4) score 계산 + 광고 URL 적재         │                          │
│           └──────────────┬───────────────────────┘                          │
│                          ▼                                                  │
│              ┌───────────────────────┐                                      │
│              │  serving.shopping_ad   │                                      │
...
1. 축제 리스트 수집 → 생성형 AI로 팝업 광고 이미지 제작 → OCI 업로드
2. 매칭 엔진:
   - `detected_object_clip` / `detected_object_stt`에서 관광지/지역 + 음식 트리거 구간 집계
   - `detected_object_yolo`에서 보조 신호 추가 (음식 bbox)
   - 관광지/지역 → 해당 지자체 광고 이미지 URL 매칭
   - 음식 → 제철장터 채널 상품 연계
   - `serving.shopping_ad`에 INSERT
3. `expires_at` 30일 지난 레코드 정리
...
- `serving.shopping_ad`에 광고 정보가 비정규화되어 있어 **JOIN 없이 단일 SELECT**로 응답
...
```
 ...
 │  │  │  score                  ← 0.0~1.0                │      │    │
 │  │  │  ad_action_type         ← local_gov_popup | seasonal_market│   │
 │  │  │  ad_image_url           ← 지자체 광고 이미지 (OCI URL)   │    │
 │  │  │  product_name           ← 제철장터 상품명                │    │
 │  │  │  channel                ← 연계 채널명 (제철장터 등)       │    │
 │  │  │  expires_at             ← TTL 30일                      │    │
 │  │  └──────────────────────────┬───────────────────────┘      │    │
 │  └─────────────────────────────┼──────────────────────────────┘    │
 │                                │                                    │
...
| 결정 | 이유 |
|------|------|
| 광고 정보 내장 | API가 JOIN 없이 **단일 SELECT**로 0.5초 폴링 응답 가능 |
| `ad_action_type` 분기 | `local_gov_popup` (지자체 광고) / `seasonal_market` (제철장터) 구분 |
