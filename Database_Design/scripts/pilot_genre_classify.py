"""
TV 연예/오락 세부장르 분류 파일럿 (100 시리즈) v2

분류 전략:
  1단계 — 안전 키워드 규칙 (의미가 명확한 키워드만 직접 분류)
  2단계 — Ollama exaone3.5 (규칙 미분류 + 모호 키워드 매칭 항목)

  '여행' 키워드는 두 종류로 분리:
    SAFE   → 즉시 여행 분류 (여행, 해외, 투어 등 의미 명확)
    AMBIGUOUS → Ollama로 위임 (나라, 세계, 도시 등 다른 맥락에서도 빈번)

세부장르: 여행 / 음식_먹방 / 뷰티 / 패션 / 쇼핑 / 기타

실행:
    python Database_Design/scripts/pilot_genre_classify.py
    python Database_Design/scripts/pilot_genre_classify.py --limit 50
"""
import sys, os, csv, json, time, re, argparse, requests, logging
sys.stdout.reconfigure(encoding='utf-8')

_root = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", ".."))
sys.path.insert(0, _root)
from dotenv import load_dotenv
load_dotenv(os.path.join(_root, ".env"))
import psycopg2

logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s", datefmt="%H:%M:%S")
log = logging.getLogger(__name__)

OLLAMA_URL   = "http://localhost:11434/api/generate"
OLLAMA_MODEL = "exaone3.5:7.8b"
OUT_PATH     = os.path.join(_root, "Database_Design", "data", "pilot_genre_100_v4.csv")
GENRES       = ["여행", "음식_먹방", "뷰티", "패션", "쇼핑",
                "음악_예능", "연애_데이팅", "토크_버라이어티", "체험_버라이어티", "관찰_리얼리티", "기타"]

# ── 1단계: 키워드 규칙 ──────────────────────────────────────────────────
# 여행: 의미 명확 → 즉시 분류
TRAVEL_SAFE_KEYWORDS = [
    "여행", "해외", "투어", "세계여행", "배낭",
    "유럽", "아시아", "일본", "중국", "유랑",
    "세계일주", "해외여행",
]
# 여행: 다른 맥락에서도 자주 쓰임 → Ollama 위임
TRAVEL_AMBIGUOUS_KEYWORDS = [
    "나라", "세계", "도시", "글로벌", "미국", "이국", "여정", "탐방",
]

KEYWORD_RULES = {
    "음식_먹방": [
        "먹방", "맛집", "음식", "요리", "셰프", "식당", "레스토랑",
        "쿡", "푸드", "food", "먹는", "먹어", "한식", "양식",
        "중식", "일식", "디저트", "밥", "이팅",
    ],
    "뷰티": [
        "뷰티", "화장", "메이크업", "makeup", "피부", "헤어", "미용",
        "beauty", "스킨케어", "네일", "성형", "코스메틱",
    ],
    "패션": [
        "패션", "fashion", "의상", "코디", "ootd",
        "스트리트패션", "런웨이", "브랜드룩", "패셔니스타",
    ],
    "쇼핑": [
        "쇼핑", "shopping", "홈쇼핑", "구매", "구입",
        "판매", "할인", "세일", "추천템",
    ],
    "음악_예능": [
        "오디션", "트로트", "힙합", "랩스타", "가요제", "뮤지션",
        "보컬", "싱어송라이터", "아이돌 오디션", "가요 프로그램", "음악 버라이어티",
    ],
    "연애_데이팅": [
        "연애", "데이팅", "소개팅", "짝을 찾", "로맨스",
        "남녀 출연자", "미팅 프로그램",
    ],
    "체험_버라이어티": [
        "미션", "서바이벌", "탈출", "체험", "도전", "특수부대",
        "밀리터리", "스턴트", "스포츠 버라이어티",
    ],
}

def keyword_classify(text: str) -> tuple[str | None, str, str]:
    """
    Returns (genre, method, matched_keyword)
    - genre: 분류 결과 or None (Ollama로 위임)
    - method: "keyword" | "ollama_ambiguous" | None
    - matched_keyword: 매칭된 키워드 (없으면 "")
    """
    t = text.lower()

    # 여행 안전 키워드 — 즉시 분류
    for kw in TRAVEL_SAFE_KEYWORDS:
        if kw in t:
            return "여행", "keyword", kw

    # 여행 모호 키워드 — Ollama 위임 플래그
    for kw in TRAVEL_AMBIGUOUS_KEYWORDS:
        if kw in t:
            return None, "ollama_ambiguous", kw

    # 나머지 장르 키워드
    for genre, kws in KEYWORD_RULES.items():
        for kw in kws:
            if kw in t:
                return genre, "keyword", kw

    return None, "ollama", ""


# ── 2단계: Ollama 분류 ──────────────────────────────────────────────────
PROMPT_TMPL = """\
다음은 TV 예능/오락 프로그램의 정보입니다.
아래 세부장르 중 가장 적합한 **하나만** 선택해서 장르명만 출력하세요. (설명 불필요)

세부장르 목록:
- 여행 (해외·국내 여행, 배낭여행, 현지 생활, 세계 일주 등 — 이동/탐험 자체가 주제)
- 음식_먹방 (맛집 탐방, 먹방, 요리, 쿡방, 식당 운영 등 — 음식이 주제)
- 뷰티 (메이크업, 스킨케어, 헤어, 성형, 미용 등)
- 패션 (스타일링, 코디, 의상, 패션쇼 등)
- 쇼핑 (홈쇼핑, 상품 소개, 구매 리뷰 등)
- 음악_예능 (가수 오디션, 가요쇼, 음악 서바이벌, 음악 버라이어티, 음악 토크, 아이돌 육성 등 — 음악이 핵심)
- 연애_데이팅 (남녀 만남, 연애, 소개팅, 짝 찾기, 결혼 등)
- 토크_버라이어티 (패널 토크, 시사·이슈 토론, 게스트 인터뷰, 추리·범죄 토크, 정보 예능 등)
- 체험_버라이어티 (미션 수행, 서바이벌, 탈출, 스포츠/레저 도전, 군/특수부대 체험, 게임 버라이어티 등)
- 관찰_리얼리티 (일상 관찰, 가족·부부·자녀 생활, 직업 체험 밀착, 리얼리티 다큐 등)
- 기타 (예능이 아닌 드라마·영화가 잘못 분류된 경우, 또는 위 항목에 해당 없음)

판단 기준:
- 프로그램의 핵심 포맷·목적을 기준으로 선택하세요.
- 여행을 배경으로 하더라도 음식이 주제면 음식_먹방, 음악이 주제면 음악_예능으로 분류하세요.
- 출연자들이 함께 다니며 미션·도전을 수행하면 체험_버라이어티입니다.
- 일상을 카메라로 관찰하는 포맷이면 관찰_리얼리티입니다.

프로그램 정보:
제목: {title}
줄거리: {smry}

세부장르:"""

def ollama_classify(title: str, smry: str) -> str:
    prompt = PROMPT_TMPL.format(title=title[:80], smry=smry[:300])
    try:
        r = requests.post(OLLAMA_URL, json={
            "model": OLLAMA_MODEL, "prompt": prompt,
            "stream": False, "options": {"temperature": 0.0, "num_predict": 10}
        }, timeout=30)
        raw = r.json().get("response", "").strip()
        # 출력에서 장르명 추출
        for g in GENRES:
            if g in raw:
                return g
        return "기타"
    except Exception as e:
        log.warning("Ollama 오류: %s", e)
        return "기타"


# ── DB 조회 ─────────────────────────────────────────────────────────────
def fetch_series(limit: int) -> list[dict]:
    conn = psycopg2.connect(
        host=os.getenv("DB_HOST"), port=int(os.getenv("DB_PORT","5432")),
        dbname=os.getenv("DB_NAME"), user=os.getenv("DB_USER"), password=os.getenv("DB_PASSWORD")
    )
    with conn.cursor() as cur:
        cur.execute("""
            SELECT DISTINCT ON (series_nm)
                full_asset_id, series_nm, smry
            FROM vod
            WHERE ct_cl = 'TV 연예/오락'
              AND series_nm IS NOT NULL
              AND smry IS NOT NULL AND smry != ''
            ORDER BY series_nm, full_asset_id
            LIMIT %s
        """, (limit,))
        rows = cur.fetchall()
    conn.close()
    return [{"id": r[0], "series_nm": r[1], "smry": r[2]} for r in rows]


# ── 메인 ─────────────────────────────────────────────────────────────────
def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--limit", type=int, default=100)
    args = parser.parse_args()

    log.info("DB 조회: TV 연예/오락 시리즈 %d건", args.limit)
    series_list = fetch_series(args.limit)
    log.info("조회 완료: %d건", len(series_list))

    results = []
    kw_count = 0
    ol_count = 0
    ol_amb_count = 0

    for i, s in enumerate(series_list, 1):
        text = f"{s['series_nm']} {s['smry']}"
        genre, method, matched_kw = keyword_classify(text)

        if genre is None:
            genre = ollama_classify(s["series_nm"], s["smry"])
            if method == "ollama_ambiguous":
                ol_amb_count += 1
            else:
                ol_count += 1
                method = "ollama"
        else:
            kw_count += 1

        results.append({
            "series_nm": s["series_nm"],
            "genre": genre,
            "method": method,
            "matched_kw": matched_kw,
            "smry_preview": s["smry"][:100].replace("\n", " "),
        })

        log.info("[%d/%d] %-30s → %-10s (%s / kw=%s)",
                 i, len(series_list), s["series_nm"][:30], genre, method, matched_kw or "-")

    # 저장
    os.makedirs(os.path.dirname(OUT_PATH), exist_ok=True)
    with open(OUT_PATH, "w", newline="", encoding="utf-8-sig") as f:
        writer = csv.DictWriter(f, fieldnames=["series_nm","genre","method","matched_kw","smry_preview"])
        writer.writeheader()
        writer.writerows(results)

    # 요약
    from collections import Counter
    dist = Counter(r["genre"] for r in results)
    log.info("=== 분류 결과 ===")
    log.info("안전키워드: %d건 / Ollama(모호키워드): %d건 / Ollama(키워드없음): %d건",
             kw_count, ol_amb_count, ol_count)
    for g in GENRES:
        log.info("  %-12s: %d건", g, dist.get(g, 0))
    log.info("저장 → %s", OUT_PATH)


if __name__ == "__main__":
    main()
