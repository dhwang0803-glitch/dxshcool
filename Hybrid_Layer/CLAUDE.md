...
| `public.vod` | `full_asset_id`, `director`, `cast_lead`, `cast_guest`, `genre`, `genre_detail`, `rating`, `ct_cl`, `series_nm` | - | vod_tag 생성 소스 + 시리즈 중복제거용 |
...
| 처리 위치 | 역할 |
|-----------|------|
| `shelf_builder.py` (Phase 4) | 시리즈 중복제거 후 tag_recommendation 적재 |
| `API_Server/recommend_service.py` | top 10 미달 태그 숨김 처리 + 후순위 태그로 대체 |

**tag_recommendation에 10개 미만이 적재된 태그가 있어도 shelf_builder에서 별도 처리하지 않는다.**
API_Server가 vod_list 10개 미달 패턴을 숨기고 충족하는 후순위 태그로 자동 대체한다.
