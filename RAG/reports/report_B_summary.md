...
## 전체 166k 벌크 파이프라인 실행 결과 (2026-03-10)

> 방식: Bulk 수평분할 파이프라인 | 대상: 166,159건 (unique 시리즈 20,096개, 8.3× dedup)
> 실행일: 2026-03-10 | 총 소요: 226.4분 (3시간 46분) | DB UPDATE: 127,315건

### 컬럼별 결과

| 컬럼 | RAG 전 | RAG 후 | 채움률 | 신규 채움 |
|------|-------:|-------:|-------:|----------:|
| director | 147,029 (88.5%) | **153,711 (92.5%)** | +4.0%p | +6,682건 |
| cast_lead | 0 (0%) | **116,547 (70.1%)** | 신규 컬럼 | +116,547건 |
| rating | 0 (0%) | **97,498 (58.7%)** | 신규 컬럼 | +97,498건 |
| release_date | 0 (0%) | **121,532 (73.1%)** | 신규 컬럼 | +121,532건 |
| smry | 166,144 (100.0%) | **166,155 (100.0%)** | +11건 | +11건 |
| series_nm | 165,728 (99.7%) | **166,045 (99.9%)** | +0.2%p | +317건 |
| disp_rtm | 165,452 (99.6%) | **165,802 (99.8%)** | +0.2%p | +350건 |

### 소스별 기여 (rag_processed=127,315건)

| 소스 | 건수 | 비율 |
|------|-----:|-----:|
| TMDB | 95,593 | 75.1% |
| TMDB+KMDB | 10,350 | 8.1% |
| 미매칭 | 10,768 | 8.5% |
| TMDB+JW | 4,297 | 3.4% |
| JustWatch | 3,255 | 2.6% |
| KMDB | 2,156 | 1.7% |
| KMDB+JustWatch | 602 | 0.5% |
| TMDB+KMDB+JW | 210 | 0.2% |
| DATA_GO / TMDB+DATA_GO | 62 | 0.0% |

### 파이프라인 성능

| Stage | 대상 시리즈 | 소요 시간 | 속도 |
|-------|----------:|----------:|-----:|
| Stage 1 TMDB | 20,096 | ~2시간 18분 | ~1.5~4 시리즈/s |
| Stage 2 KMDB | ~9,600 | ~내부 처리 | — |
| Stage 3 JustWatch | 12,064 | ~22분 | ~5~8 시리즈/s |
| Stage 4 DATA_GO | 11,230 | ~40분 | ~4.7 시리즈/s |
| DB UPDATE | 166,159건 | ~60분 | 배치 500건 |

### 잔존 결측치 및 한계

| 컬럼 | 잔존 NULL | 주요 원인 |
|------|----------:|----------|
| rating | 68,661건 | TV 방송 등급 = 방심위 관할, 4개 소스 모두 미보유 |
| cast_lead | 49,612건 | 키즈·교육·로컬 전용 콘텐츠, 외부 DB 미등록 |
| release_date | 44,627건 | 동일 |
| director | 12,448건 | 예능·키즈 감독 개념 부재 |

### 다음 개선 포인트
1. **TV 방송 rating** — 방심위(방송통신심의위원회) API 별도 연동
2. **키즈/로컬 콘텐츠** — 추가 데이터 소스 발굴 필요
3. **_tmdb_search 2-phase 최적화** — 26→9 HTTP 호출 (65% 감소)로 Stage 1 속도 개선 확인
