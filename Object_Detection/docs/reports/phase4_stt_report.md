...
## 파일럿 실행 결과 (2026-03-15, VOD 10건)

```
실행: python scripts/batch_stt_score.py --limit 10 --random \
      --input-dir ../VOD_Embedding/data/trailers_아름
```
...
| 처리 VOD 수 | 10건 |
| 성공 / 실패 | 10 / 0 |
| STT 구간 수 (마지막 VOD 기준) | 86개 |
| 총 키워드 매칭 건수 | **2건** |
| 매칭 키워드 | `한우` 1건, `전주` 1건 |
| 카테고리 분포 | 지방특산물 1건, 여행지 1건 |

### 분석

파이프라인 자체는 정상 동작 (10/10 성공, 오류 없음).
그러나 **10개 VOD에서 키워드 매칭 2건은 recall이 극히 낮은 수준**이다.

**원인**: `stt_keywords.yaml`이 시드 수준(카테고리별 5~10개)으로,
실제 예능·드라마 대사에서 등장하는 품목명을 거의 커버하지 못함.

예를 들어 "한우"는 매칭됐지만 "소고기", "육회", "투뿔", "등심" 등 동의어·하위어는 미등록.
방언, 줄임말, 발화 중 노이즈로 인한 Whisper 인식 오류도 recall 저하 요인.
...
| 한계 | 내용 |
|------|------|
| 키워드 recall 극히 낮음 | 10VOD 기준 2건 — 시드 키워드만으로는 실운영 불가 |
| 동의어·하위어 미등록 | "한우"는 있어도 "소고기", "등심", "육회" 없음 |
| 방언·발음 오류 | Whisper 인식 결과가 표준어와 다르면 매칭 실패 |
| Whisper 처리 속도 | small 모델 기준 1분 영상 → 약 30~60초 (CPU) |

### 권장 후속 작업

1. **키워드 확장**: Shopping_Ad 담당자와 협의 후 카테고리별 50개 이상으로 `stt_keywords.yaml` 확충
   - 동의어·하위어·브랜드명 포함
2. **형태소 분석 도입**: 정확한 매칭을 위해 키워드 단순 `in` 검색 → 형태소 기반으로 개선 검토
