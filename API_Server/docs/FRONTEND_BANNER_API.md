...
> 운영 서버(`vod-api`)와 혼동 주의. 개발 테스트는 반드시 `vod-api-dev`로 호출.
...
| **TOP10 배너에 추천 문구** | `rec_reason`, `rec_sentence` 필드 추가 (nullable) |
...
| 필드 | 타입 | 설명 |
|------|------|------|
| `top_vod` | `object \| null` | 최상위 추천 VOD (히어로 영역) |
| `top_vod.series_id` | `string` | full_asset_id |
| `top_vod.asset_nm` | `string` | 자산명 |
| `top_vod.poster_url` | `string \| null` | 포스터 URL (세로형) |
| `top_vod.backdrop_url` | `string \| null` | 가로형 배경 이미지 URL (히어로 배너용). **backdrop_url IS NOT NULL인 VOD만 반환** |
| `patterns[].pattern_rank` | `int` | 패턴 순서 (1부터) |
| `patterns[].pattern_reason` | `string` | 패턴 설명 라벨 (아래 참고) |
| `patterns[].vod_list[].series_id` | `string` | full_asset_id |
...
| 조건 | `source` | 내용 |
|------|----------|------|
| 개인화 데이터 있음 | `"personalized"` | top_vod + 태그/cold/벡터 패턴 |
| 개인화 데이터 없음 | `"popular_fallback"` | top_vod = 인기 1위, patterns = 인기 2~10위 |
...
```
source: "personalized"
top_vod: poster_url 있는 hybrid 최상위 1개
patterns: 7개
