...
### 10.1.1 세그먼트별 분화 예시 — 슬기로운 의사생활

동일 VOD에 대해 5개 세그먼트가 각기 다른 톤으로 문구를 생성한 대표 사례:

| 세그먼트 | 레이블 | rec_sentence | 톤 |
|----------|--------|-------------|-----|
| 0 | 키즈/애니 | 20년 우정의 조정석, 유연석, 정경호, 김대명! | 캐스팅 어필 |
| 1 | 예능/버라이어티 | 조정석, 유연석, 정경호, 김대명, 20년 우정은 여전하지만 병원에서는 각자의 하루를 살아간다! 웃음과 감동이 넘실대는 병원 사람들의 이야기. | 일상/힐링 |
| 2 | 액션/범죄/성인 | 생과 사의 경계, 숨 막히는 의료 현장의 하루. 조정석, 유연석, 정경호, 김대명이 펼치는 20년 우정 속 은밀한 갈등과 진실을 마주하다. | 의료 긴장감/갈등 |
| 3 | 가족 시청층 | 조정석, 유연석 등 20년 지기 친구들이 펼치는 따뜻한 이야기. | 간결한 감성 요약 |
| 4 | 메인스트림 드라마 | 20년 우정의 유쾌한 시작! 조정석, 유연석 등 친구들은 병원에서 웃음과 감동을 번갈아 가며 선사한다. | 유쾌/코미디 |

> 같은 작품이지만 세그먼트 1은 "웃음과 감동", 세그먼트 2는 "생과 사의 경계, 은밀한 갈등", 세그먼트 3은 "따뜻한 이야기"처럼 사용자 클러스터 취향에 맞게 소구 포인트가 명확히 분화된다.
...
| 자동 평가 지표 산출 | **대기** | seed 데이터(Gold Standard) 기반 BLEU/ROUGE/BERTScore 산출 필요. seed jsonl 파일은 Colab/별도 저장소에 보관 중 — 가져온 뒤 reference vs generated 쌍 비교 실행. 발표용 성능비교표 제작 목적 |
